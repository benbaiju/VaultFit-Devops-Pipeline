...
type Props = { size?: VaultFitLogoSize; tone?: "default" | "admin" };

const wordmarkSize = { sm: 17, md: 21, lg: 27 } as const;

export function VaultFitLogo({ size = "md", tone = "default" }: Props) {
  const s = SIZE[size];
  const w = markWidthForHeight(s.h);
  const accent = tone === "admin" ? "#2ea043" : colors.vaultTeal;
  const wm = wordmarkSize[size];

  return (
    <View style={styles.row} accessibilityRole="header" accessibilityLabel="VaultFit">
      <Svg width={w} height={s.h} viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} preserveAspectRatio="xMidYMid meet">
        <Path d={DUMBBELL_PATH_D} stroke={accent} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" fill="none" />
      </Svg>
      <View style={styles.wordmark}>
        <Text style={[styles.vault, { fontSize: wm, fontFamily: Font.interExtraBold }]}>Vault</Text>
        <Text style={[styles.fit, { fontSize: wm, fontFamily: Font.interExtraBold, color: accent }]}>Fit</Text>
      </View>
    </View>
  );
}
